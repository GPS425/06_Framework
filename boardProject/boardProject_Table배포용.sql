...
CREATE TABLE "MEMBER" (
	...
	"AUTHORITY"	NUMBER	DEFAULT 1	NOT NULL	-- 1: 일반회원, 2: 관리자
);
...
-- 부모 댓글 1의 자식 댓글
INSERT INTO "COMMENT"	
VALUES( SEQ_COMMENT_NO.NEXTVAL, '부모 1의 자식 1입니다 으헤헤',
			  DEFAULT, DEFAULT,	1995, 3, 2001);
			 
INSERT INTO "COMMENT"	
VALUES( SEQ_COMMENT_NO.NEXTVAL, '부모 1의 자식 2입니다 으하하',
			  DEFAULT, DEFAULT,	1995, 3, 2001);
			 
			 
-- 부모 댓글 2의 자식 댓글			 
INSERT INTO "COMMENT"	
VALUES( SEQ_COMMENT_NO.NEXTVAL, '부모 2의 자식 1 레츠고',
			  DEFAULT, DEFAULT,	1995, 3, 2002);
			 
-- 부모 댓글 2의 자식 1의 자식 댓글			 
INSERT INTO "COMMENT"
VALUES( SEQ_COMMENT_NO.NEXTVAL, '오늘점심뭐먹을지추천좀요감사합니다',
			  DEFAULT, DEFAULT,	1995, 3, 2006);
...
WHERE COMMENT_DEL_FL = 'N' -- 댓글 자체가 삭제되어지지 않았거나
OR 0 != (SELECT COUNT(*) FROM "COMMENT" SUB
				WHERE SUB.PARENT_COMMENT_NO = C.COMMENT_NO
				AND COMMENT_DEL_FL = 'N')
-- 자식 댓글 중 삭제되어지지 않은 행이 하나라도 있다면 부모 댓글을 조회 대상에 포함하겠다(부모 댓글이 삭제되었더라도 "삭제된 댓글입니다"라고 표기되도록)
-- 현재 댓글(C.COMMENT_NO)을 부모로 하는 자식 댓글 중 삭제되지 않은 댓글의 행의 갯수 카운트
-- >> 삭제되지 않은 댓글이거나, 삭제된 댓글이라도 그 아래에 삭제되지 않은 자식 댓글이 존재하면 댓글이 조회되도록 하는 목적의 SQL문
...
JOIN "BOARD_TYPE" T USING(BOARD_CODE)
WHERE (SELECT COUNT(*) FROM "BOARD_LIKE" L WHERE L.BOARD_NO = B.BOARD_NO) = (
  SELECT MAX(COUNT(*))
  FROM "BOARD_LIKE"
  GROUP BY BOARD_NO
)
