...
/*
 * AOP(Aspect-Oriented-Programming) 분산되어있는 관심사/관점을 모듈화시키는 기법
 * - 주요 비즈니스 로직과 관련이 없는 부가적인 기능을 추가할 때 유용하다
 * ex) 코드 중간중간 로그 찍을 때, 트랜잭션 처리하고 싶을 때, 보안 처리 추가 등...
 * 
 * 주요 어노테이션
 * - @Aspect : Aspect를 정의하는 데 사용되는 어노테이션, 클래스 상단에 작성
...
@Component	// 클래스 상단에 작성, Bean 등록
// @Aspect	// 공통 관심사가 작성된 클래스임을 명시함(AOP 동작용 클래스)
...
public class TestAspect {
	...
	// * : 모든 리턴 타입을 나타냄 (아무거나 상관없다는 뜻)
	// edu.kh.project : 패키지 3레벨
	// .. : 0개 이상의 하위 패키지
	// *Controller* : 이름에 "Controller"라는 문자열을 포함한 모든 클래스를 대상으로 함
	// .* : 모든 메서드를 나타냄
	...
	public void ControllerEnd(JoinPoint jp) {
							//JoinPoint : AOP 기능이 적용된 대상을 말함
		
		// AOP가 적용된 클래스 이름 얻어오기
		String className = jp.getTarget().getClass().getSimpleName();
		// ex) MainController, MainController처럼 깔끔하게 나옴
