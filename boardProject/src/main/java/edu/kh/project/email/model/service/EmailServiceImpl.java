...
public class EmailServiceImpl implements EmailService{
	...
	// SpringTemplateEngine : 타임리프를 이용해서 html  코드 >> Java 코드로 변환
	...
	public String sendEmail(String type, String email) {
		...
		try {
			...
			// > true : 파일 첨부를 사용할 것인지 여부를 지정함
			// (파일 첨부 및 내부 이미지를 삽입 가능하게 함)
			// - "UTF-8" : 이메일 내용이 UTF-8 인코딩으로 전송됨
			
			// 메일 기본 정보 세팅
			helper.setTo(email); // 받는 사람(수신자)
			helper.setSubject("[boardProject] 회원가입 인증번호"); // 제목
