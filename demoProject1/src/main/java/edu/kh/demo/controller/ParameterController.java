...
@Slf4j // log를 이용한 메세지 콘솔창에 출력할 때 사용(lombok 제공)
public class ParameterController {
	...
	public String paramTest2(@RequestParam("title") String title,
		...
		return "redirect:/param/main";	// /param/main 주소 GET 방식
	}
	...
	// @ModelAttribute를 이용한 파라미터 얻어오기 (중요)
	// @ModelAttribute : DTO 또는 VO와 같이 사용하는 어노테이션
	...
	// >> 아무 어노테이션도 안 붙였을 경우 스프링 자체적으로 데이터 타입을 보고 판단
	// > 단순 타입(String, int, Integer 등) >> @RequestParam으로 인식
	// > 나머지 복잡한 객체(DTO, VO 등) >> @ModelAttribute로 인식
	// >>> 가독성을 위해 웬만하면 작성하는 것을 추천함.
	@PostMapping("test4")
	public String paramTest4(/*@ModelAttribute*/ Member inputMember) {
		
		log.debug("inputMember: " + inputMember);
		
		return "redirect:main";	// 상대 경로 작성법
		// /param/main
		// 상대 경로에서 중요한 부분 >> 현재 위치!!
		// >> 현재 경로의 가장 마지막 레벨의 주소값을
		// "redirect:이 주소" 로 변경하기 때문
